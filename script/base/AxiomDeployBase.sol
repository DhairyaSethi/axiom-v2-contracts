...
import {GOERLI_CHAIN_ID, CHIADO_CHAIN_ID, MAINNET_CHAIN_ID, SEPOLIA_CHAIN_ID} from "../../contracts/libraries/configuration/AxiomV2Configuration.sol";
...
import {TestHeaderVerifier} from "../../contracts/query/TestHeaderVerifier.sol";
...
address constant MAINNET_TIMELOCK = address(
    0x0c8630ef55ADC50266189CF2Ea2222A1a86Fcea2
);
address constant MAINNET_GUARDIAN = address(
    0x99C7E4eB11541388535a4608C14738C24f131921
);
address constant MAINNET_UNFREEZE = address(
    0x99C7E4eB11541388535a4608C14738C24f131921
);
address constant MAINNET_CORE_PROVER = address(
    0x8018fe32fCFd3d166E8b4c4E37105318A84BA11b
);
address constant MAINNET_QUERY_PROVER = address(
    0x8018fe32fCFd3d166E8b4c4E37105318A84BA11b
);

address constant GOERLI_TIMELOCK = address(
    0xc2d7e38a40808BBfc1834C79b5Ba4b27bC4c462e
);
address constant GOERLI_GUARDIAN = address(
    0x99C7E4eB11541388535a4608C14738C24f131921
);
address constant GOERLI_UNFREEZE = address(
    0x99C7E4eB11541388535a4608C14738C24f131921
);
address constant GOERLI_CORE_PROVER = address(
    0x8018fe32fCFd3d166E8b4c4E37105318A84BA11b
);
address constant GOERLI_QUERY_PROVER = address(
    0x8018fe32fCFd3d166E8b4c4E37105318A84BA11b
);

address constant GOERLI_OLD_CORE_MOCK_ADDRESS = address(
    0x0c8630ef55ADC50266189CF2Ea2222A1a86Fcea2
);
address constant GOERLI_V08_CORE_MOCK_ADDRESS = address(
    0x2aE6ad6127C222f071C023086C442080B03AfCCe
);
address constant GOERLI_V08_CORE_HISTORICAL_MOCK_ADDRESS = address(
    0x071b7aA74f060B40cB7EEE577c30E634276e7C9f
);

address constant CHIADO_TIMELOCK = address(
    0x58d1D3400cD08E22983cbC2C0AE9AaE13cc5efef
);
address constant CHIADO_GUARDIAN = address(
    0x58d1D3400cD08E22983cbC2C0AE9AaE13cc5efef
);
address constant CHIADO_UNFREEZE = address(
    0x58d1D3400cD08E22983cbC2C0AE9AaE13cc5efef
);
address constant CHIADO_CORE_PROVER = address(
    0x58d1D3400cD08E22983cbC2C0AE9AaE13cc5efef
);
address constant CHIADO_QUERY_PROVER = address(
    0x58d1D3400cD08E22983cbC2C0AE9AaE13cc5efef
);
address constant CHIADO_V08_CORE_HISTORICAL_MOCK_ADDRESS = address(
    0x58d1D3400cD08E22983cbC2C0AE9AaE13cc5efef
);
...
abstract contract AxiomDeployBase is Script {
    ...
    function _getCREATE3Addr(uint64 sourceChainId) internal returns (address) {
        if (sourceChainId == MAINNET_CHAIN_ID) {
            ...
            return address(0x93FEC2C00BfE902F733B57c5a6CeeD7CD1384AE1);
        } else if (sourceChainId == CHIADO_CHAIN_ID) {
            // return address(0x824d4f8299B9495BD1B145C5b95aB1530220Ce8F);
            return address(_deployCREATE3());
        } else {
            revert("AxiomDeployBase: invalid sourceChainId _getCREATE3Addr");
        }
    }
    ...
    function _getCoreAddress(
        ...
        if (sourceChainId == MAINNET_CHAIN_ID) {
            ...
            coreAddress = CHIADO_V08_CORE_HISTORICAL_MOCK_ADDRESS;
        } else {
            revert("AxiomDeployBase: invalid sourceChainId _getCoreAddress");
        }
    }

    function _getCoreMockAddress(
        uint64 sourceChainId
    ) internal returns (address coreAddress) {
        if (sourceChainId == GOERLI_CHAIN_ID) {
            ...
            revert(
                "AxiomDeployBase: invalid sourceChainId _getCoreMockAddress"
            );
        }
    }

    function _getMultisigAddresses(
        uint64 sourceChainId
    )
        internal
        returns (
        ...
        )
    {
        if (sourceChainId == MAINNET_CHAIN_ID) {
            ...
            timelock = CHIADO_TIMELOCK;
            guardian = CHIADO_GUARDIAN;
            unfreeze = CHIADO_UNFREEZE;
            coreProver = CHIADO_CORE_PROVER;
            queryProver = CHIADO_QUERY_PROVER;
        } else {
            revert("AxiomDeployBase: invalid sourceChainId");
        }
    }

    function _getAggregateVkeyHashes(
        uint32 version
    ) internal returns (bytes32[] memory aggregateVkeyHashes) {
        if (version == 7) {
            aggregateVkeyHashes = new bytes32[](1);
            aggregateVkeyHashes[0] = bytes32(
                0x0e237205617f6abafb336441b10ed85bc213cadcb008e65440981e4c2188c488
            );
        } else if (version == 8) {
            // contract-v0.8
            ...
            // config 1
            aggregateVkeyHashes[0] = bytes32(
                0x0739285abf85e3891e07b6ffa68cbd41b7557c51894d99a153c386fe3161e900
            );
            // config 2
            aggregateVkeyHashes[1] = bytes32(
                0x094fc9e4526a20304cfa40b4f9c2410a6edb7e23a29c491527cd3d4babd882d3
            );
        } else if (version == 9) {
            // contract-v0.9
            aggregateVkeyHashes = new bytes32[](2);
            // config 1
            aggregateVkeyHashes[0] = bytes32(
                0x0088c85dd433925fd2c5f083fc523be447fa7c5046a93425f0ef3df3523768f5
            );
            // config 2
            aggregateVkeyHashes[1] = bytes32(
                0x1b46de31e41b181c8f38639a52ee6af6936f2cf067d628fda6ee768f217f565a
            );
        }
    }

    function _deployCoreVerifiers(
        uint64 sourceChainId
    )
        internal
        returns (address verifierAddress, address historicalVerifierAddress)
    {
        ...
        historicalVerifierAddress = _deployCoreHistoricalVerifier(
            sourceChainId
        );
    }

    function _deployCoreVerifier(
        uint64 sourceChainId
    ) internal returns (address verifierAddress) {
        if (sourceChainId == MAINNET_CHAIN_ID) {
            ...
            revert(
                "AxiomDeployBase: invalid sourceChainId _deployCoreVerifier"
            );
        }
    }

    function _deployCoreHistoricalVerifier(
        uint64 sourceChainId
    ) internal returns (address historicalVerifierAddress) {
        YulDeployer deployer = new YulDeployer();
        vm.allowCheatcodes(address(deployer));
        if (sourceChainId == MAINNET_CHAIN_ID) {
            historicalVerifierAddress = address(
                deployer.deployContract("core/mainnet_17_7.v1")
            );
        } else if (sourceChainId == GOERLI_CHAIN_ID) {
            historicalVerifierAddress = address(
                deployer.deployContract("core/goerli_17_7.v1")
            );
        } else {
            revert(
                "AxiomDeployBase: invalid sourceChainId _deployCoreHistoricalVerifier"
            );
        }
    }
    ...
    function _deployHeaderVerifier(
        ...
        AxiomV2HeaderVerifier headerImpl;
        if (sourceChainId == CHIADO_CHAIN_ID) {
            headerImpl = AxiomV2HeaderVerifier(
                address(new TestHeaderVerifier(sourceChainId))
            );
        } else {
            headerImpl = new AxiomV2HeaderVerifier(sourceChainId);
        }
        ...
        AxiomProxy headerProxy = new AxiomProxy(
            address(headerImpl),
            headerInit
        );
    ...
    function _deployProver(
        ...
        AxiomProxy proverProxy = new AxiomProxy(
            address(proverImpl),
            proverInit
        );
    ...
    function _deployResultStore(
        ...
        AxiomProxy resultProxy = new AxiomProxy(
            address(resultImpl),
            resultInit
        );
    ...
    function _deployQuery(
        IAxiomV2Query.AxiomV2QueryInit memory init,
        bytes32 salt
    ) internal returns (address) {
        ...
        address addr = create3.deploy(
            salt,
            abi.encodePacked(
                type(AxiomProxy).creationCode,
                abi.encode(address(queryImpl), queryInit)
            )
        );
