...
contract AxiomV2Prover is IAxiomV2Prover, AxiomAccess, UUPSUpgradeable {
    ...
    function fulfillQuery(
        ...
        IAxiomV2Query(axiomQueryAddress).fulfillQuery(
            mmrWitness,
            computeResults,
            proof,
            callback,
    ...
    function addAllowedProver(
        bytes32 querySchema,
        address target,
        address prover
    ) external onlyRole(TIMELOCK_ROLE) {
        allowedProvers[keccak256(abi.encodePacked(querySchema, target))][
            prover
        ] = true;
    ...
    function supportsInterface(
        ...
        return
            interfaceId == type(IAxiomV2Prover).interfaceId ||
